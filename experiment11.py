...
def _next_batch(train_labels, batch_size, index_in_epoch, mask,num):
    ...
    for i in labels:        
        if i==0:
            img = cleardata.unit_circle(224, 80, mask)
        elif i==1:
            img = shapedatagauss.unit_circle(224, 80, mask,num)
            # img = np.multiply(fractal4, img)
        elif i==2:
            img = shapfracdata.unit_circle(224, 80, mask,num)
...
def train():
    ...
    for netname in netlist:
        ...
        for epoch in range(1):
            ...
            '''  
        #保存网络结构
        torch.save(net,os.path.join('/media/liqiang/windata/project/classification/plugin/model','ex11'+netname+'_'+'net.pkl'))        
        print('预测标签：{}, 真实标签：{}'.format(pred, labels))
        y1 = Accuracy_list
        y2 = Loss_list
        Allacc.append(y1)
    ...
    plt.ylabel('Accuracy')
    plt.savefig(os.path.join('/media/liqiang/windata/project/classification/plugin/result','ex11'+'train_'+"accuracy.jpg"))
    plt.show()
    plt.close()
    fig = plt.figure()
    ...
    # plt.yscale('log')
    # plt.ylim([-0.01,0.06])         
    plt.savefig(os.path.join('/media/liqiang/windata/project/classification/plugin/result','ex11'+'train_'+"loss.jpg"))
    plt.show()
    plt.close()
