...
def train():
    ...
    plt.ylabel('Accuracy')
    plt.savefig(os.path.join('/media/liqiang/windata/project/classification/plugin/result','train_'+"accuracy.jpg"))
    plt.show()
    plt.close()
    for i in range(len(netlist)):
    ...
    plt.ylabel('Loss')  
    plt.yscale('log')         
    plt.savefig(os.path.join('/media/liqiang/windata/project/classification/plugin/result','train_'+"loss.jpg"))
    plt.show()
    plt.close()
    '''
    ###保存验证集训练曲线
