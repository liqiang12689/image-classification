...
def train():
    ...
    for netname in netlist:
        ...
        # Train the model
        y0 = np.zeros(1500,dtype=np.int)
        y1 = np.ones(1500, dtype=np.int)
        y2 = np.ones(1500, dtype=np.int)*2
        ...
        for epoch in range(1):
            ...
            for iters in range(int(y_label.__len__()/batch_size)):
                ...
                tempdata = np.reshape(batch_x,(batch_size*2, 1, 224, 224))
                ...
                if running_correct.item()/(batch_size*2*batch) > 0:
                    print("Batch {}, Train Loss:{:.6f}, Train ACC:{:.4f}".format(
                    batch, running_loss/(batch_size*2*batch), running_correct.item()/(batch_size*2*batch)))
                    # print('预测标签：{}, 真实标签：{}'.format(pred, labels))
                maxacc.append(running_correct.item()/(batch_size*2*batch))
                Accuracy_list.append(running_correct.item()/(batch_size*2*batch))
                Loss_list.append(running_loss/(batch_size*2*batch))
            ...
            '''  
        #保存网络结构
        torch.save(net,os.path.join('/media/liqiang/windata/project/classification/plugin/model','ex9'+netname+'_'+'net.pkl'))        
        print('预测标签：{}, 真实标签：{}'.format(pred, labels))
        y1 = Accuracy_list
        y2 = Loss_list
        Allacc.append(y1)
    ...
    plt.ylabel('Accuracy')
    plt.savefig(os.path.join('/media/liqiang/windata/project/classification/plugin/result','ex9'+'train_'+"accuracy.jpg"))
    plt.show()
    plt.close()
    fig = plt.figure()
    ...
    # plt.yscale('log')
    # plt.ylim([-0.01,0.06])         
    plt.savefig(os.path.join('/media/liqiang/windata/project/classification/plugin/result','ex9'+'train_'+"loss.jpg"))
    plt.show()
    plt.close()
