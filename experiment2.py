...
def train():
    ...
    for netname in netlist:
        ...
        for epoch in range(1):
            ...
            for iters in range(int(y_label.__len__()/batch_size)):
                ...
                newmask = (mask+np.max(mask))/(np.max(mask)-np.min(mask))
                batch_x, batch_y, index_in_epoch = _next_batch(y_label, batch_size, index_in_epoch,newmask)
            ...
            '''  
        #保存网络结构
        torch.save(net,os.path.join('/media/liqiang/windata/project/classification/plugin/model','ex2'+netname+'_'+'net.pkl'))        
        print('预测标签：{}, 真实标签：{}'.format(pred, labels))
        y1 = Accuracy_list
        y2 = Loss_list
        Allacc.append(y1)
    ...
    plt.ylabel('Accuracy')
    plt.savefig(os.path.join('/media/liqiang/windata/project/classification/plugin/result','ex2'+'train_'+"accuracy.jpg"))
    plt.show()
    plt.close()
    fig = plt.figure()
    ...
    # plt.yscale('log')
    # plt.ylim([-0.01,0.06])         
    plt.savefig(os.path.join('/media/liqiang/windata/project/classification/plugin/result','ex2'+'train_'+"loss.jpg"))
    plt.show()
    plt.close()
    '''
    ###保存验证集训练曲线
