...
def train():
    ...
    for netname in netlist:
        ...
        for epoch in range(1):
            ...
            '''  
        #保存网络结构
        torch.save(net,os.path.join('/media/liqiang/windata/project/classification/plugin/model','ex6'+netname+'_'+'net.pkl'))        
        print('预测标签：{}, 真实标签：{}'.format(pred, labels))
        y1 = Accuracy_list
        y2 = Loss_list
        Allacc.append(y1)
    ...
    plt.ylabel('Accuracy')
    plt.savefig(os.path.join('/media/liqiang/windata/project/classification/plugin/result','ex6'+'train_'+"accuracy.jpg"))
    plt.show()
    plt.close()
    fig = plt.figure()
    ...
    # plt.yscale('log')
    # plt.ylim([-0.01,0.06])         
    plt.savefig(os.path.join('/media/liqiang/windata/project/classification/plugin/result','ex6'+'train_'+"loss.jpg"))
    plt.show()
    plt.close()
